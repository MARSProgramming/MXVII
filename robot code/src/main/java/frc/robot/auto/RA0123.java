...
public class RA0123 extends SequentialCommandGroup{
    public RA0123(DrivetrainSubsystem drivetrainSubsystem, IntakeWheels intakeWheels, IntakePivot intakePivot, ShooterFlywheel shooterFlywheel, ThePivot thePivot, Limelight ll, PhotonVision pv){
        ...
        addCommands(
            ...
            (new DriveAtPath(drivetrainSubsystem, RA1, ll, false, pv).alongWith(new IntakeCommand(intakePivot, intakeWheels, thePivot).withTimeout(2))).andThen(new DriveAtPath(drivetrainSubsystem, R1B, ll, false, pv).deadlineWith(intakePivot.setPositionCommand(() -> 0, false)))
            ...
            new DriveAtPath(drivetrainSubsystem, RB2, ll, false, pv).alongWith(new IntakeCommand(intakePivot, intakeWheels, thePivot)).deadlineWith(shooterFlywheel.runVelocity(() -> DynamicConstants.ShooterFlywheel.idleVelocity))
            .withTimeout(3),
            new IntegratedShooterCommand(intakeWheels, shooterFlywheel, thePivot, drivetrainSubsystem).withTimeout(3).unless(() -> !intakeWheels.hasPiece()),
            new DriveAtPath(drivetrainSubsystem, R23, ll, false, pv).alongWith(new IntakeCommand(intakePivot, intakeWheels, thePivot)).deadlineWith(shooterFlywheel.runVelocity(() -> DynamicConstants.ShooterFlywheel.idleVelocity))
            .withTimeout(3),
